...
if ($x && $y) {
	...
	// Update the database...
	require_once (MYSQL);
	$q = "UPDATE users SET active=1 WHERE (email='" . mysqli_real_escape_string($dbc, $x) . "' AND active='" . mysqli_real_escape_string($dbc, $y) . "') LIMIT 1";
	$r = mysqli_query ($dbc, $q) or trigger_error("Query: $q\n<br />MySQL Error: " . mysqli_error($dbc));
	
	// Print a customized message:
	if (mysqli_affected_rows($dbc) == 1) {
		echo "<h3>تم تفعيل حسابك الآن. بإمكانك تسجيل الدخول الآن.</h3>";
	} else {

...
{
	$query = "
	SELECT * FROM st_statistical_system 
	...
	{
		$output .= '<option value="'.$row["id_statistical"].'">'.$row["Department"].'</option>';
	}
	return $output;
}
...
{
	$query = "SELECT * FROM st_department_table 
	WHERE department_status = 'active' 
	AND id_statistical = '".$category_id."'
	ORDER BY nameDepartment ASC";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
...
{
	$query = "
	SELECT * FROM employees WHERE user_status='active'";
	$statement = $connect->prepare($query);
	$statement->execute();
...
{
	$query = "
	SELECT * FROM st_statistical_system WHERE category_status='active'
	";
	$statement = $connect->prepare($query);
	$statement->execute();
	return $statement->rowCount();
...
{
	$query = "
	SELECT * FROM st_department_table WHERE department_status='active'
	";
	$statement = $connect->prepare($query);
	$statement->execute();
	return $statement->rowCount();
